...
// Set, view, or clear the single active goal shown as a progress bar on Insights.
...
export default function GoalAdmin() {
  ...
  function load() {
    fetch('/api/goal').then((response) => response.json()).then(setGoal).finally(() => setLoading(false));
  }

  useEffect(() => {
    fetch('/api/goal').then((response) => response.json()).then((data) => {
      setGoal(data);
      setLoading(false);
    });
  }, []);
        ...
        <form onSubmit={handleSet}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div>
              ...
              <input value={targetUnits} onChange={(event) => setTargetUnits(event.target.value)} inputMode="decimal" placeholder="e.g. 50" style={{ ...INPUT, fontFamily: 'var(--font-mono)' }} required />
            </div>
            <div>
              <label style={LABEL}>Deadline</label>
              <input type="date" value={deadline} onChange={(event) => setDeadline(event.target.value)} style={INPUT} required />
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={LABEL}>Note (optional)</label>
            <input value={note} onChange={(event) => setNote(event.target.value)} placeholder="e.g. Save for a holiday" style={INPUT} />
          </div>
