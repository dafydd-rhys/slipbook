// Persistence layer for every data type the app stores (bets, bankroll,
// templates, filter presets, goal, cached AI summary). Backed by Upstash
// Redis when configured, falling back to local JSON files in data/ for
...
// Same as finite(), but drops the value entirely instead of substituting a fallback.
...
// Sanitizes a bet's own odds/stake/returns, and every one of its legs.
...
// Connects to Upstash Redis if its env vars are set, otherwise signals "use the local-JSON fallback".
...
function createStore<T>(redisKey: string, localFileName: string, empty: T) {
  ...
  // Reads the stored blob, falling back to `empty` if nothing's been written yet.
  ...
  // Persists the whole blob, creating the local data/ folder on first write if needed.
...
// True once a soft-deleted bet has sat in the trash past its retention window.
...
async function readAllBets(): Promise<BetsData> {
  ...
  const kept = bets.filter((bet) => !isExpiredTrash(bet));

  if (kept.length !== bets.length) {
    await writeBets({ bets: kept });
  }
...
export async function readBets(): Promise<BetsData> {
  ...
  return { bets: bets.filter((bet) => !bet.deletedAt) };
}

// Soft-deleted bets only — what the admin Trash tab shows.
...
const writeBets = betsStore.write;

// Adds a single bet, newest-first.
...
// Merges `updates` into the existing bet with this id, re-sanitizing the result.
export async function updateBet(id: string, updates: Partial<Bet>): Promise<Bet | null> {
  const data = await readAllBets();
  const idx = data.bets.findIndex((bet) => bet.id === id);
...
// Un-marks a soft-deleted bet, moving it back out of the trash.
export async function restoreBet(id: string): Promise<boolean> {
  const data = await readAllBets();
  const idx = data.bets.findIndex((bet) => bet.id === id);

  if (idx === -1 || !data.bets[idx].deletedAt) {
    return false;
  }
...
export async function purgeBet(id: string): Promise<boolean> {
  const data = await readAllBets();
  const idx = data.bets.findIndex((bet) => bet.id === id && !!bet.deletedAt);
...
const writeBankroll = bankrollStore.write;

// Adds a single bankroll entry (deposit/withdrawal/adjustment), newest-first.
...
// Removes a bankroll entry by id.
...
const writeTemplates = templatesStore.write;

// Saves a new bet template, newest-first.
...
// Removes a bet template by id.
export async function deleteTemplate(id: string): Promise<boolean> {
  const data = await readTemplates();
  const idx = data.templates.findIndex((template) => template.id === id);
...
const writePresets = presetsStore.write;

// Saves a new filter preset, newest-first.
...
// Removes a saved filter preset by id.
export async function deletePreset(id: string): Promise<boolean> {
  const data = await readPresets();
  const idx = data.presets.findIndex((preset) => preset.id === id);
